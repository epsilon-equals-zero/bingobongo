import { List, Login, Logout } from "@mui/icons-material";
import {
    Avatar,
    CircularProgress,
    IconButton,
...
function UserView() {
    ...
    return (
        <>
            ...
            >
                <MenuItem onClick={redirectToGamesList}>
                    <ListItemIcon>
                        <List />
                    </ListItemIcon>
                    My Games
    ...
    return (
        <Box {...props} sx={{ ...sx, display: "flex", justifyContent: "flex-end" }}>
            {loading ? (
                ...
                <Tooltip title="Login">
                    <IconButton href={`/login?redirect=${encodeURI(urlPath)}`}>
                        <Login sx={{ width: 32, height: 32, color: (theme: Theme) => theme.palette.primary.main }} />
                    </IconButton>
                </Tooltip>
            )}
        </Box>
